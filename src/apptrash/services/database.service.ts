...
//Prod Base URL
const baseUrl = 'http://160.94.105.82:3305/api/';
...
export class DatabaseService {
  ...
  // Anthony Altered
  getGoTerms(tissues: string[], cell_types: string[], pathway: string, comparisonType: string) {
  ...
  staticDownload(sample_ids: number[]): void {
    const urls = sample_ids.map(id => `http://160.94.105.82:3305/static/Sample_${id}.tar.gz`);
